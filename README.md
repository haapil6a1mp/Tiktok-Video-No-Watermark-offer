...
### 1.获取列表详情：```https://www.tikwm.com/api/mix/info``` ```[GET]```

### 参数  ```url```

### 2.获取播放列表中的视频：```https://www.tikwm.com/api/mix/posts``` ```[GET]```

### 参数  ```mix_id```, ```count```, ```cursor```

### 3.获取用户的列表详情：```https://www.tikwm.com/api/mix/list``` ```[GET]```

### 参数  ```unique_id``` or ```user_id```, ```count```, ```cursor```
...
### 1.获取集合详情：```https://www.tikwm.com/api/collection/info``` ```[GET]```

#### 参数  ```url```

### 2.获取集合中的视频：```https://www.tikwm.com/api/collection/posts``` ```[GET]```

#### 参数  ```collection_id```, ```count```, ```cursor```

### 3.获取用户的集合：```https://www.tikwm.com/api/collection/list``` ```[GET]```
